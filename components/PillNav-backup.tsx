...
const PillNav: React.FC<PillNavProps> = ({
  ...
  useEffect(() => {
    ...
    if (initialLoadAnimation) {
      ...
      // Ensure logo stays at normal size - no scaling animation
      if (logo) {
        gsap.set(logo, { scale: 1, transform: "none" });
      }

      // Disable nav items width animation to prevent layout shifting
      if (navItems) {
        gsap.set(navItems, { width: "auto", overflow: "visible" });
      }
    } else {
      // Even without animation, ensure logo is properly sized
      const logo = logoRef.current;
      if (logo) {
        gsap.set(logo, { scale: 1, transform: "none" });
      }

      // Ensure nav items are properly sized
      const navItems = navItemsRef.current;
      if (navItems) {
        gsap.set(navItems, { width: "auto", overflow: "visible" });
      }
